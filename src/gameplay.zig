...
var enemyDie: r.Sound = undefined;
var playerShoot: r.Sound = undefined;
var won: r.Sound = undefined;
...
pub fn Init() void {
    ...
    score = 0;
    alpha = 0;
    won = r.LoadSound("resources/audio/won.wav");
    ...
    playerShoot = r.LoadSound("resources/audio/player_shoot.wav");
    ...
    enemyDie = r.LoadSound("resources/audio/light_off.wav");
    ...
    for (0..gl.NUM_SHOOTS) |i| {
        ...
        bullet[i].rec.height = 8;
        bullet[i].speed.x = 7;
        bullet[i].speed.y = 0;
        bullet[i].isAlive = false;
        bullet[i].color = r.DARKBLUE;
    }
}
...
pub fn Draw() void {
    if (!isGameOver) {
        ...
        for (0..@as(usize, @intCast(aliveEnemies))) |i| {
            if (enemy[i].isAlive) r.DrawRectangleRec(enemy[i].rec, enemy[i].color);
        }
...
pub fn Unload() void {
    ...
    r.UnloadSound(enemyDie);
    r.UnloadSound(playerShoot);
    r.UnloadSound(won);
}
